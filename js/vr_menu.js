...
function initVRMenu() {
    ...
    var menuMaterial = new THREE.MeshBasicMaterial({ map: vrMenuTexture, transparent: true });
...
function updateVRMenuCanvas() {
    ...
    vrMenuContext.fillText('A/X: Reset | B/Y + ThumbClick: Giant Meteor', 30, 120);
    
    vrMenuContext.fillText('Time Speed: ' + timeScale + 'x (Use Thumbstick)', 30, 170);
    
    vrMenuContext.fillText('Meteor: Grip', 30, 220);
...
function updateVRControllers() {
    ...
    for (var i = 0; i < gamepads.length; i++) {
        ...
        if (menuState.isMenuVisible) {
            ...
            if ((gp.buttons[0] && gp.buttons[0].pressed) || (gp.buttons[4] && gp.buttons[4].pressed)) {
                if (now - menuState.lastButtonPress > debounceTime) {
            ...
            if ((gp.buttons[2] && gp.buttons[2].pressed) || (gp.buttons[3] && gp.buttons[3].pressed)) {
                if (now - menuState.lastButtonPress > 1000) { // 1 second cooldown
            ...
            if (xAxe > 0.5) { // Right
                if (now - menuState.lastButtonPress > 100) { // Faster repeat for stick
                    timeScale += 500;
