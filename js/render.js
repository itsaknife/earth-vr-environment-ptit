...
function shatterEarth() {
  ...
  if (!doomsdayFlash) {
    ...
    var flashMat = new THREE.MeshBasicMaterial({ color: 0xffddaa, transparent: true, opacity: 1.0, blending: THREE.AdditiveBlending });
  ...
  if (!planetShockwave) {
    ...
    var ringMat = new THREE.MeshBasicMaterial({ color: 0xff8822, transparent: true, opacity: 0.9, side: THREE.DoubleSide, blending: THREE.AdditiveBlending });
  ...
  var impactNorm = doomsdayMeteorGroup ? doomsdayMeteorGroup.position.clone().normalize() : new THREE.Vector3(0,1,0); 
  if (earthChunks.length === 0) {
    var chunkGeo = new THREE.IcosahedronGeometry(0.5, 0); // Rough, low-poly rocks
    var colors = [0x555555, 0xff3300, 0x113355, 0x332211, 0xffaa00, 0x222222, 0x004400]; // Multi-biome fragment colors
  ...
  if (!earthDebris) {
    ...
    var debrisMat = new THREE.PointsMaterial({ color: 0xff6611, size: 0.2, transparent: true, opacity: 0.8, blending: THREE.AdditiveBlending });
  ...
  for (var i = 0; i < earthDebris.geometry.vertices.length; i++) {
    ...
    p.set(Math.random() - 0.5, Math.random() - 0.5, Math.random() - 0.5).normalize().multiplyScalar(Math.random() * 6.3);
  ...
  if (typeof earthObject !== 'undefined') {
    ...
    if (earthCraters.length > 0) {
      for (var i = 0; i < earthCraters.length; i++) {
          ...
          if (earthCraters[i].material.map) earthCraters[i].material.map.dispose();
...
function createMushroomCloudTexture() {
  ...
  for(var i = 0; i < 15; i++) {
     ...
     puffGrad.addColorStop(0, Math.random() > 0.5 ? 'rgba(255,100,0,0.6)' : 'rgba(50,50,50,0.8)');
...
function createCrater(impactPos) {
  ...
  for(var i = 0; i < 8; i++) {
    ...
    cCtx.lineTo(128 + (endX-128)*0.5 + (Math.random()-0.5)*20, 128 + (endY-128)*0.5 + (Math.random()-0.5)*20);
    cCtx.lineTo(endX, endY);
    cCtx.stroke();
  }
  
  var craterTex = new THREE.CanvasTexture(cCanvas);
  var craterMat = new THREE.MeshBasicMaterial({ map: craterTex, transparent: true, depthWrite: false, side: THREE.DoubleSide });
...
function launchMeteor() {
  if (!meteorGroup) {
    ...
    var ringMat = new THREE.MeshBasicMaterial({ color: 0xff8800, transparent: true, opacity: 1.0, side: THREE.DoubleSide, blending: THREE.AdditiveBlending });
    ...
    var flashMat = new THREE.MeshBasicMaterial({ map: new THREE.CanvasTexture(flashCanvas), transparent: true, depthWrite: false, side: THREE.DoubleSide, blending: THREE.AdditiveBlending });
...
function initSceneObjects() {
  ...
  var nightSphereMesh = new THREE.Mesh(nightSphereGeometry, nightSphereMaterial);
